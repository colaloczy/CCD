...
from pygments.lexers import CSharpLexer  # Import the C# lexer from pygments
...
def get_file_pairs(program_dir, samples_per_folder=50):
    ...
    for folder in folders:
        ...
        if os.path.isdir(folder_path):
            python_folder = os.path.join(folder_path, 'C#')  # 假设每个文件夹下有C#子文件夹
            if os.path.exists(python_folder):  # 检查'C#'子文件夹是否存在
                all_files = [f for f in os.listdir(python_folder) if f.endswith('.cs')]
...
def get_code_block(file_path):
    try:
        ...
        # Remove comments: handle both single-line (//) and multi-line (/* */) comments
        content = re.sub(r'//.*?\n|/\*.*?\*/', '', content, flags=re.DOTALL)
        
        # Remove preprocessor directives (if any, such as #define, #include)
        content = re.sub(r'#.*\n', '', content)

        # Remove empty lines
        content = "\n".join([line for line in content.splitlines() if line.strip()])
        
        # Split the code into tokens (variable names, operators, etc.)
        # This regex handles variable names, keywords, operators, and other symbols in C#
...
def getCodeBlock_type(file_path, return_types_only=False):
    """
    Extract tokens or token types from a C# source file using Pygments lexer.
    
    Args:
        file_path (str): Path to the source file
        return_types_only (bool): Whether to return only token types
    
    Returns:
        list: Tokens or token types
    """
    ...
    # Tokenize using Pygments C# Lexer
    tokens_and_types = []
    for token in lex(source, CSharpLexer()):
        token_type = token[0]
        token_value = token[1]
        
        if return_types_only:
            # Convert the token type to a string (e.g., "Token.Name.Function")
            token_type_str = str(token_type)  # Converts token type to string
            tokens_and_types.append(token_type_str)
        else:
            # Append both token value and token type
            tokens_and_types.append((token_value, str(token_type)))  # Convert token type to string
...
def main():
    program_dir = "C#CodeNet"  # 数据集路径
    # output_dir = " "
    samples_per_folder = 600  # 每个文件夹采样600个文件
    ...
    final_df.to_csv('cs_token.csv')
    print(f"Saved features to cs_token.csv")
